...
char* indexes(const char* path)
{
    //set pointers for the sought out file names. 
    char *a = "/index.html";
    char *b = "/index.php";
    char *indexhtml = malloc(strlen(path) + strlen(a) + 1);
    strcpy(indexhtml, path);
    strcat(indexhtml, a);
    char *indexphp = malloc(strlen(path) + strlen(b) + 1);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // check if file is null, initialize length and content
    *length = 0;
    *content = NULL;
    ...
    // iterate over the contents of the file, read 512 chunks of 1 byte at a time
    // read while it can (not 0) the update will be to read again and store in buffer
    for(int c = fread(buffer, 1, 512, file); c != 0; c = fread(buffer, 1, 512, file))
    {
        // first iteration it will allocate memory other iterations will reallocate mem, the new size will be:
        ...
        *content = realloc(*content, *length + c + 1);
        ...
        // copy the buffer into content destination is the first byte + old bytes
        // from buffer new bytes c
...
{
    //const char find = ".";
    //Possible corner case of having multiple extensions is not addressed in this code.
...
{
    // TODO
    //Verify that the clients socket is open
    if (cfd == -1)
    {
        return -1;
    }
    ...
    {
        i++;
        for (int z = 0; target[i] != ' ' && target[i] != i++, z++)
        {
            query[z] = target[i];
        }
    }
